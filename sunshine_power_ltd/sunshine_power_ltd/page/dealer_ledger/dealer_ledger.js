	...
	function render_table(rows) {
		...
		const body_rows = rows.map(r => {
						...
						<table class="nested-table">
							...
							${r.items.map(i => {
								...
								const purchase_cols = can_view_purchase_price ? `
									<td class="num">${i.purchase_price ? esc(fmt_cur(i.purchase_price)) : "—"}</td>
									<td class="num">${tp ? esc(fmt_cur(tp)) : "—"}</td>` : "";
								return `
								<tr>
			...
			<tr class="parent-row" data-id="${esc(r.invoice_name)}">
				...
				${can_view_purchase_price ? `<td class="num">${r.total_purchase_price ? esc(fmt_cur(r.total_purchase_price)) : "—"}</td>` : ""}
		...
		<table class="dl-tbl">
			<thead>
				...
				<tr>
					...
					${can_view_purchase_price ? `<th class="num">Purchase Price</th>` : ""}
